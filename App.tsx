...
const APP_URL = "https://dishguise.app"; 
...
// --- PROGRESSION TITLES ---
// Expanded title range for slower progression
const getTitle = (level: number) => {
    if (level < 5) return "Dishwasher";
    if (level < 10) return "Prep Cook";
    if (level < 20) return "Commis Chef";
    if (level < 35) return "Chef de Partie";
    if (level < 50) return "Sous Chef";
    if (level < 75) return "Chef de Cuisine";
    return "Executive Chef";
};

// --- XP LOGIC ---
// Calculates current level based on total XP using a progressive curve
// Level 1: 0-2999
// Level 2: 3000-6999 (+4000 gap)
// Level 3: 7000-11999 (+5000 gap)
const calculateLevelFromXP = (totalXP: number) => {
    ...
    while (totalXP >= xpForNext) {
        level++;
        gap += 1000; // Each level requires 1000 more XP than the last gap
        xpForNext += gap;
    }
    return level;
};
...
const App: React.FC = () => {
  ...
  // Leveling State
  const [startLevel, setStartLevel] = useState(userProfile.level);
  ...
  // Handle Game Over Level Check
  useEffect(() => {
      if (appState === AppState.GAME_OVER) {
          if (userProfile.level > startLevel) {
              setShowLevelUpModal(true);
          }
      }
  }, [appState, userProfile.level, startLevel]);
  ...
  const addToCollection = (dishName: string, score: number, flagEmoji: string) => {
      setUserProfile(prev => {
  ...
  const startGame = async (selectedDiff: Difficulty) => {
    ...
    // Track starting level for this session
    setStartLevel(userProfile.level);
  ...
  const handleRoundWin = (score: number) => {
      ...
      awardXP(score);
      if (currentRoundData) {
          addToCollection(currentRoundData.targetDish, score, currentRoundData.flagEmoji);
      }
      setGameResult(prev => {
        if (!prev) return null;
        return {
  ...
  return (
    <div className="h-screen bg-[#121212] flex flex-col overflow-hidden relative">
       ...
       {tutorialJustFinished && gameMode === 'DETECTIVE' && (
           <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-40 pointer-events-none flex flex-col items-center animate-bounce">
